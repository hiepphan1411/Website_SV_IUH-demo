...
function renderTableHeader(maxThuongXuyen, maxThucHanh) {
  ...
  const totalCols = 12 + maxThuongXuyen + maxThucHanh;
...
function renderTableRow(mon, stt, maxThuongXuyen = 3, maxThucHanh = 3) {
  ...
  for (let i = 0; i < maxThuongXuyen; i++) {
    const score = mon.diem.thuongXuyen[i];
    const displayScore = score !== undefined ? score : "-";
    const style = getScoreStyle(score);
    thuongXuyenCells += `<td class="text-center" ${style}>${displayScore}</td>`;
  }

  let thucHanhCells = "";
  for (let i = 0; i < maxThucHanh; i++) {
    const score = mon.diem.thucHanh[i];
    const displayScore = score !== undefined ? score : "-";
    const style = getScoreStyle(score);
    thucHanhCells += `<td class="text-center" ${style}>${displayScore}</td>`;
  }

  const isPass = mon.trangThai === "Đạt" ? true : false;

  return `
    <tr class="${isPass ? "" : "improvement-row"}">
      <td class="text-center">${stt}</td>
      <td class="text-center">${mon.maMonHoc}</td>
      <td class="subject-name">${mon.tenMonHoc}</td>
      <td class="text-center">${mon.soTinChi}</td>
      <td class="text-center" ${getScoreStyle(mon.diem.giuaKy)}>${mon.diem.giuaKy || "-"}</td>
      ${thuongXuyenCells}
      ${thucHanhCells}
      <td class="text-center" ${getScoreStyle(mon.diem.cuoiKy)}>${mon.diem.cuoiKy || "-"}</td>
      ...
      <td class="text-center">
        <span class="ranking-badge ${getGradeBadgeClass(mon.diem.diemChu)}">${mon.xepLoai}</span>
      </td>
      <td class="text-center">
...
function renderSummaryRow(semester, totalCols = 17) {
  ...
  const getRankingClass = (xepLoai) => {
    ...
    if (ranking.includes("giỏi")) return "good";
    if (ranking.includes("khá")) return "fair";
