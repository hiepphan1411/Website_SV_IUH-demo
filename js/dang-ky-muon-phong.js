const dropdownData = {
  ...
  statuses: ["Đề xuất", "Đã xác nhận", "Đã duyệt", "Không xác nhận", "Từ chối"],
};
...
function editRecord(element) {
  ...
  if (data) {
    ...
    document.getElementById("modalTuTiet").value = data.tiet.split(" - ")[0];
    document.getElementById("modalDenTiet").value = data.tiet.split(" - ")[1];
    ...
    document.getElementById("modalDayNha").value = "Trung tâm GDTX Hải Phòng";
    document.getElementById("modalPhong").value = "Trung tâm GDTX Hải Phòng";
...
function filterReservations() {
  ...
  const filteredData = reservationData.filter((item) => {
    if (filters.fromPeriod && filters.toPeriod) {
      const itemPeriods = item.tiet.split(" - ").map((p) => parseInt(p.trim()));
  ...
  if (btnSaveReservation) {
    btnSaveReservation.addEventListener("click", function () {
      ...
      const newReservation = {
        ...
        thoiGianBatDau: document.getElementById("modalThoiGianBatDau").value,
        thoiGianKetThuc: document.getElementById("modalThoiGianKetThuc").value,
        ...
        lyDoMuonPhong: document.getElementById("modalLyDoMuonPhong").value,
      };
