...
/* Load semester data from sessionStorage  */
...
function initializeSearch() {
    $('#tableSearch').on('keyup', function () {
        ...
        console.log('[v0] Searching with keyword:', keyword);

        $('.detail-table tbody tr').filter(function () {
            $(this).toggle($(this).text().toLowerCase().indexOf(keyword) > -1);
        });
    });
}

function initializeKendoUI() {
    try {
        if (window.kendo) {
            console.log('[v0] Kendo UI available');
        }
    } catch (error) {
        console.log('[v0] Kendo UI not available:', error.message);
    }
}

function initializeJQueryUI() {
    try {
        console.log('[v0] jQuery UI initialized');
    } catch (error) {
        console.log('[v0] jQuery UI initialization note:', error.message);
    }
}
...
/**
 * Highlight rows with specific criteria
 */
...
/**
 * Collapse all rows except headers
 */
...
// Initialize all category rows on page load
document.addEventListener('DOMContentLoaded', function () {
    const categoryRows = document.querySelectorAll('.category-row');

    // Set first category (I) as expanded by default
